...
DATA_DIR = Path(r"C:\Users\Administrator\Documents\Mecrado\Automation\tools\创建xlsx\xlsx文件\Shopee")
...
def fetch():
    with ENGINE.begin() as conn:
        p = pd.read_sql(text("SELECT id AS folder_id, sku_folder FROM product_folder"), conn)
        s = pd.read_sql(text("SELECT sku_code, folder_id, color_desc, size_desc FROM sku"), conn)
        i = pd.read_sql(text("SELECT folder_id, sku_code, file_path, img_role FROM image_asset"), conn)
...
def build(lines, prod, sku, img):
    ...
    for raw in lines:
        ...
        if pf.folder_id in ig.groups:
            ...
            pics = (g[g.img_role == "main"].file_path.tolist() + g[g.img_role == "detail"].file_path.tolist())[:10]
        else:
        ...
        if pf.folder_id in ig.groups:
            ...
            opt = opt[opt.img_role == "option"].merge(sku[["sku_code", "color_desc"]], on="sku_code", how="left")
        ...
        if pf.folder_id in sg.groups:
            ...
            if uniq.empty:
                ...
                for row in uniq.itertuples():
                    ws_i.append([pid, row.sku_code, row.size_desc, row.color_desc, color_img.get(row.color_desc, "")])
        else:
            logging.warning(f"No SKUs for folder {pf.folder_id} ({raw})")
            ws_i.append([pid, "", "", "", ""])
        pid += 1
