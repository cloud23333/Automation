...
def classify(fn):
    ...
    role = (
        "main"
        if MAIN_KEY.search(base)
        else (
            "size"
            if SIZE_KEY.search(base)
            else "option" if OPTION_KEY.search(base) else None
        )
    )
    if not role:
        return None
    tag = (
        "qty"
        if role == "option" and QTY_KEY.search(base)
        else "noqty" if role == "option" and NOQTY_KEY.search(base) else None
    )
...
def agg(recs, root):
    ...
    for (fc, st, sf), imgs in g.items():
        ...
        qty = sum(
            1 for i in imgs if i["img_role"] == "option" and i["option_tag"] == "qty"
        )
        noqty = sum(
            1 for i in imgs if i["img_role"] == "option" and i["option_tag"] == "noqty"
        )
        ...
        row = dict(
            ...
            风格=st,
            SKU文件夹=sf,
            文件夹地址=os.path.join(root, fc, st, sf),
            主图数=c.get("main", 0),
            尺寸图数=c.get("size", 0),
            选项图数=c.get("option", 0),
            带数量张数=qty,
...
def main():
    ...
    bad = df1[
        (df1["主+尺寸<5"] == 1) | (df1["选项图有问题"] == 1) | (df1["有坏SKU"] == 1)
    ]
